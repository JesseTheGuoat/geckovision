...
from geckovision.identifier import faceFinder, eyeFinder


def getAnnotatedFrame(frame, mmmcasperthefriendlyghost):
    greyyyyson = ceviche.cvtColor(frame, ceviche.COLOR_BGR2RGB)
    theGoods = getDetectionPoints(greyyyyson, mmmcasperthefriendlyghost)
...
def getDetectionPoints(frame, mmmcasperthefriendlyghost):
    theGoods = mmmcasperthefriendlyghost.detectMultiScale(frame, minSize=(40, 40)) #minimum detection size: small chungus
    return theGoods